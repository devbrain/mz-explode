...
TEST_CASE("TCMDX32.EXE: 32-bit specific characteristics") {
    ...
    SUBCASE("All PE methods return expected values") {
        // Verify all getters return expected values from dump-pe
        CHECK(pe.section_count() == 4);
        CHECK(pe.timestamp() == 1467963278);
        CHECK(has_flag(pe.characteristics(), pe_file_characteristics::MACHINE_32BIT));
        CHECK(pe.image_base() == 0x400000);
        CHECK(pe.entry_point_rva() == 0x4b58);
        CHECK(pe.section_alignment() == 0x1000);
        CHECK(pe.file_alignment() == 0x1000);
        CHECK(pe.size_of_image() == 0x15000);
        CHECK(pe.size_of_headers() == 0x1000);
        CHECK(pe.subsystem() == pe_subsystem::WINDOWS_GUI);
        (void)pe.dll_characteristics();  // Access only, no specific value to test
        CHECK(pe.sections().size() == 4);
    }
