...
format_type executable_factory::detect_format(std::span<const uint8_t> data) {
    ...
    // Check MZ signature
    uint16_t mz_sig = libexe::format::read_uint16_le(ptr, end);
    ptr -= 2;  // Rewind for full parse

    if (mz_sig != libexe::format::DOS_SIGNATURE) {
    ...
    try {
        ...
        uint16_t ext_sig = libexe::format::read_uint16_le(ptr, end);
        ptr -= 2;  // Rewind

        // Check for NE signature (0x4E45 = 'NE')
        if (ext_sig == libexe::format::NE_SIGNATURE) {
