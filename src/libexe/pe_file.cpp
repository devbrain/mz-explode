...
pe_file::~pe_file() = default;
pe_file::pe_file(pe_file&&) noexcept = default;
pe_file& pe_file::operator=(pe_file&&) noexcept = default;
...
pe_file pe_file::from_data_source(std::unique_ptr<data_source> source) {
    if (!source || source->size() < 64) {  // Minimum for DOS stub + PE header pointer
        throw std::runtime_error("File too small to be a valid PE executable");
    }
...
std::optional<rich_header> pe_file::rich() const {
    ...
    rich_header_ = rich_header_parser::parse(data_->span(), pe_offset_);
...
bool pe_file::has_rich_header() const {
    ...
    return rich_header_parser::has_rich_header(data_->span(), pe_offset_);
}
...
bool pe_file::has_overlay() const {
    if (!overlay_parsed_) {
        overlay_info_ = overlay_detector::detect(
            data_->span(), pe_offset_, section_count_, optional_header_size_
        );
...
std::span<const uint8_t> pe_file::overlay_data() const {
    ...
    return overlay_detector::view(data_->span(), overlay_info_);
}
