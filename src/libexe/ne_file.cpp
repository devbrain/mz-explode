...
ne_file::ne_file() = default;
ne_file::~ne_file() = default;
ne_file::ne_file(ne_file&&) noexcept = default;
ne_file& ne_file::operator=(ne_file&&) noexcept = default;
...
ne_file ne_file::from_data_source(std::unique_ptr<data_source> source) {
    if (!source || source->size() < 64) {  // Minimum for DOS stub + NE header pointer
        throw std::runtime_error("File too small to be a valid NE executable");
    }
