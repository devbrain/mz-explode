...
namespace explode
{
	knowledge_dynamics::knowledge_dynamics(input_exe_file& inp)
		...
		m_exe_file(inp)
	{
		offset_type exe_data_start = inp [exe_file::HEADER_SIZE_PARA] * 16L;

		offset_type extra_data_start = inp [exe_file::NUM_OF_PAGES] * 512L;
		if (inp[exe_file::NUM_OF_BYTES_IN_LAST_PAGE])
		{
			extra_data_start -= (512 - inp[exe_file::NUM_OF_BYTES_IN_LAST_PAGE]);
		}
		...
		{
			extra_data_start2 -= (512 - inner[exe_file::NUM_OF_BYTES_IN_LAST_PAGE]);
		}
